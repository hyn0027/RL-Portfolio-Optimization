...
def stock_close_prices(stock_name):
    """return a list containing stock close prices from a .csv file"""
    df = pd.read_csv(os.path.join("../data", stock_name + ".csv"))
...
def maximum_drawdown(portfolio_values):
    ...
    return (
        portfolio_values[end_index] - portfolio_values[beginning_index]
    ) / portfolio_values[beginning_index]
...
def evaluate_portfolio_performance(agent, logger):
    ...
    logger.info(
        "Sharpe Ratio adjusted with Treasury bond daily return: {:.3f}".format(
            sharpe_ratio(np.array(agent.return_rates)),
            risk_free=treasury_bond_daily_return_rate(),
        )
    )
    logger.info(
        "Maximum Drawdown:        {:.3f}%".format(
...
def buy_and_hold_benchmark(stock_name, agent):
    ...
    buy_and_hold_return = (
        buy_and_hold_portfolio_values.iloc[-1] - agent.initial_portfolio_value
    )
...
def plot_portfolio_performance_comparison(stock_name, agent):
    ...
    plt.plot(
        ...
        color="blue",
        label="{} Buy and Hold Total Return: ${:.2f}".format(
            stock_name, buy_and_hold_return
        ),
    )
    # compare with S&P 500 performance in 2018
    if "^GSPC" not in stock_name:
        dates, GSPC_buy_and_hold_portfolio_values, GSPC_buy_and_hold_return = (
            buy_and_hold_benchmark("^GSPC_2018", agent)
        )
        plt.plot(
            dates,
            GSPC_buy_and_hold_portfolio_values,
            color="red",
            label="S&P 500 2018 Buy and Hold Total Return: ${:.2f}".format(
                GSPC_buy_and_hold_return
            ),
        )
